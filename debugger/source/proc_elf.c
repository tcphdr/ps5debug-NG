...
static uint64_t proc_call_remote_sce(int pid)
{
    ...
    int read_b = (int)pt_read_i(pid, (void *)(scratch + 0x0C));
...
int sys_proc_call_remote_func(int pid, void *elf_buf, uint64_t a3_unused,
                               void *thread_name, uint64_t *out_entry)
{
    (void)a3_unused;
    ...
    if (out_entry) *out_entry = (uint64_t)loaded;
...
{
    return sys_proc_call_remote_func((int)pid, (void *)elf, length, NULL, NULL);
}

long sys_proc_elf_rpc(unsigned long pid, unsigned long elf, unsigned long length,
                      uint64_t *out_entry)
{
    return sys_proc_call_remote_func((int)pid, (void *)elf, length, NULL, out_entry);
}
