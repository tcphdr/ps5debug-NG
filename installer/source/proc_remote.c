...
int elev_save_and_set(struct elev_state *st) {
    int self = getpid();
    st->valid = 0;
    st->saved_authid = kernel_get_ucred_authid_fast(self);
    if (st->saved_authid == 0) return -1;
    if (kernel_get_ucred_caps_fast(self, st->saved_caps) != 0) return -1;
    if (kernel_set_ucred_authid_fast(self, 0x4800000000010003ULL) != 0) return -1;
    uint8_t all_ones[16];
    memset(all_ones, 0xFF, 16);
    if (kernel_set_ucred_caps_fast(self, all_ones) != 0) {
        kernel_set_ucred_authid_fast(self, st->saved_authid);
        return -1;
    }
    st->valid = 1;
    return 0;
...
void elev_restore(struct elev_state *st) {
    ...
    int self = getpid();
    kernel_set_ucred_authid_fast(self, st->saved_authid);
    kernel_set_ucred_caps_fast(self, st->saved_caps);
    st->valid = 0;
}
